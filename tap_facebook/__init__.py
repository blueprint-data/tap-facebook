...
def main_impl():
    ...
    if args.discover:
        ...
        # Meltano sometimes doesn't apply select filters correctly
        # during the first run (no cached state yet). This fallback
        # ensures the tap always syncs something instead of silently
        # producing 0 records.
        if not any(s.is_selected() for s in catalog.streams):
            LOGGER.warning(
                "No streams were explicitly selected in the catalog. "
                "Auto-selecting all streams and properties to prevent silent 0-record sync."
            )
            for stream_entry in catalog.streams:
                # Mark stream as selected (breadcrumb [])
                stream_entry.metadata = metadata.write(
                    stream_entry.metadata, (), "selected", True
                )
                # Mark all properties as selected
                mdata_map = metadata.to_map(stream_entry.metadata)
                for breadcrumb in [b for b in mdata_map if len(b) == 2]:
                    stream_entry.metadata = metadata.write(
                        stream_entry.metadata, breadcrumb, "selected", True
                    )
